...
namespace JulianSangillo.Reflection {
    ...
    public class TypeCollection : ITypeCollection {
        ...
        public TypeCollection(IEnumerable<Type> types) => this.types = types;
        ...
        /// <inheritdoc />
        public Type WithName(string name) => types.FirstOrDefault(type => type.Name == name);

        /// <inheritdoc />
        public ITypeCollection AssignableTo<T>(bool interfaces = false, bool abstracts = false)
            where T : class =>
            AssignableTo(typeof(T));
        ...
        public IEnumerable<T> InstanceOf<T>()
            where T : class =>
            AssignableTo<T>().Select(t => Activator.CreateInstance(t) as T);

        /// <inheritdoc />
        public IEnumerable<object> InstanceOf(Type type) => AssignableTo(type).Select(Activator.CreateInstance);
